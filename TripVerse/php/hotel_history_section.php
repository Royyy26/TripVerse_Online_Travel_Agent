<div class="hotel-history-container">
    <div class="history-title">
        <span>Hotel Terakhir Dilihat</span>
        <!-- OPSIONAL: Tombol "Hapus semua" (bisa diuncomment jika mau) -->
        <?php /* if (!empty($hotel_history)): ?>
            <button class="btn-clear" onclick="if(confirm('Hapus semua riwayat hotel?')) window.location.href='hotel.php?clear_hotel_history=1'">
                Hapus semua
            </button>
        <?php endif; */ ?>
    </div>
    ...
    <?php if (empty($hotel_history)): ?>
        <div class="text-muted">Belum ada riwayat hotel</div>
    <?php else: ?>
...
function deleteHotelHistory(hotelId, event) {
    ...
    if (confirm('Hapus hotel ini dari riwayat?')) {
