...
// Log for debugging
error_log("=== REJECT SUPPLIER REQUEST START ===");
...
if ($currentStatus == 'rejected' || $currentStatus == 2) {
    ...
    echo json_encode(['success' => false, 'message' => 'Supplier is already rejected']);
    $conn->close();
    exit;
}
...
if ($currentStatus == 'approved' || $currentStatus == 1) {
    ...
    echo json_encode(['success' => false, 'message' => 'Supplier is already approved. Please contact admin to change status.']);
    $conn->close();
    exit;
}

// Update supplier status to rejected (approved_by dan approved_at tetap NULL)
$sql = "UPDATE user SET approved = 'rejected', approved_by = NULL, approved_at = NULL WHERE id_user = ? AND role = 'owner'";
...
if ($stmt->execute()) {
    ...
    if ($logCheck && $logCheck->num_rows > 0) {
        ...
        $logSql = "INSERT INTO activity_log (user_id, action_type, action_description, entity_type, entity_id, entity_name, ip_address, user_agent) 
                   VALUES (?, 'reject_supplier', ?, 'user', ?, ?, ?, ?)";
        $logStmt = $conn->prepare($logSql);
        
        if ($logStmt) {
            $actionDesc = "Rejected supplier: " . $supplierName . " (ID: " . $supplierId . ")";
    ...
    error_log("=== REJECT SUPPLIER REQUEST SUCCESS ===");
    echo json_encode([
        'success' => true,
    ...
    echo json_encode(['success' => false, 'message' => 'Failed to reject supplier: ' . $stmt->error]);
}
